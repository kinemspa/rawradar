...
body{font-family:'Inter',system-ui,sans-serif;background:#06060e;color:#e4e4e7;overflow-x:hidden;font-size:16px;line-height:1.5}
.text-xs{font-size:13px!important}
.text-sm{font-size:14px!important}
.tab{transition:all 0.3s ease;font-size:14px;padding:10px 20px!important}.tab:hover{background:rgba(255,255,255,0.04)}
...
<a href="/" class="flex items-center gap-3 group"><div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold group-hover:scale-105 transition-transform" style="box-shadow:0 0 20px rgba(59,130,246,0.2)">RR</div><div><h1 class="text-xl font-bold tracking-tight" style="background:linear-gradient(135deg,#e4e4e7 0%,#a1a1aa 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent">RawRadar</h1><p class="text-xs text-zinc-600 -mt-0.5 tracking-wider uppercase font-medium">Australian Climate Data</p></div></a>
...
<div class="flex-1 min-w-[200px]"><label class="text-xs text-zinc-500 mb-1.5 block font-medium">Station</label>
<select id="stn" class="w-full bg-zinc-800/50 text-zinc-200 px-4 py-3 rounded-2xl text-sm border border-white/5"></select></div>
<div class="flex-1 min-w-[300px]"><label class="text-xs text-zinc-500 mb-1.5 block font-medium">Year Range</label>
<div class="flex items-center gap-3">
<select id="yr-s-sel" class="bg-zinc-800/50 text-zinc-200 px-3 py-3 rounded-2xl text-sm border border-white/5 w-24"></select>
<div class="flex-1 relative pt-4 pb-2">
<div class="flex justify-between text-xs text-zinc-600 mb-1"><span>1910</span><span id="yr-range-label" class="text-zinc-400 font-medium">2014 — 2024</span><span>2024</span></div>
...
<select id="yr-e-sel" class="bg-zinc-800/50 text-zinc-200 px-3 py-3 rounded-2xl text-sm border border-white/5 w-24"></select>
</div></div>
...
function renderMonthly(){
  ...
  ['jan','jul'].forEach((m,mi)=>{const month=mi===0?1:7;
    ...
    CH[`month-${m}`]=new Chart($(`chart-month-${m}`),{type:'line',data:{labels:years.map(y=>y.y),datasets:[{label:'Annual',data:ll,borderColor:'#22d3ee',backgroundColor:'rgba(34,211,238,0.06)',fill:true,pointRadius:0.5,tension:0.2},{label:'10yr avg',data:trend,borderColor:'#f97316',borderWidth:2,pointRadius:0,tension:0.3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#a1a1aa',font:{size:11},usePointStyle:true}}},scales:{x:{grid:{display:false},ticks:{color:'#52525b',font:{size:11}}},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}})});
}
...
function renderRecords(){
  ...
  CH.recH=new Chart($('chart-rec-high'),{type:'bar',data:{labels:dl,datasets:[{data:dk.map(dk=>Math.max(...Object.values(decades[dk].years).flat())),backgroundColor:'rgba(239,68,68,0.6)',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false},ticks:{color:'#52525b',font:{size:11}}},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
  CH.recL=new Chart($('chart-rec-low'),{type:'bar',data:{labels:dl,datasets:[{data:dk.map(dk=>Math.min(...Object.values(decades[dk].years).flat())),backgroundColor:'rgba(59,130,246,0.6)',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false},ticks:{color:'#52525b',font:{size:11}}},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
  CH.recR=new Chart($('chart-rec-range'),{type:'bar',data:{labels:dl,datasets:[{data:dk.map((dk,i)=>Math.max(...Object.values(decades[dk].years).flat())-Math.min(...Object.values(decades[dk].years).flat())),backgroundColor:'rgba(168,85,247,0.6)',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false},ticks:{color:'#52525b',font:{size:11}}},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
  const hotDays={};
...
function renderScatter(){
  ...
  CH.scatter=new Chart($('chart-scatter'),{type:'scatter',data:{datasets:[{data:pts,backgroundColor:'rgba(59,130,246,0.12)',pointRadius:2,borderColor:'rgba(59,130,246,0.3)'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{title:{display:true,text:'Min Temp (\u00b0C)',color:'#71717a',font:{size:12}},grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}},y:{title:{display:true,text:'Max Temp (\u00b0C)',color:'#71717a',font:{size:12}},grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
  ...
  CH.dist=new Chart($('chart-dist'),{type:'bar',data:{labels:bl.map(b=>b+'\u00b0'),datasets:[{data:bl.map(b=>bins[b].length),backgroundColor:'rgba(139,92,246,0.4)',borderRadius:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false},ticks:{color:'#52525b',font:{size:11}}},y:{grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
}
...
async function renderCompareChart(){
  ...
  CH.compare=new Chart($('chart-compare'),{type:'line',data:{datasets},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:'#a1a1aa',font:{size:12},usePointStyle:true}}},scales:{x:{type:'category',grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}},y:{title:{display:true,text:'Anomaly (\u00b0C)',color:'#71717a',font:{size:12}},grid:{color:'rgba(255,255,255,0.03)'},ticks:{color:'#52525b',font:{size:11}}}}}});
}
...
function renderTable(){
  ...
  $('tbl').innerHTML=`<table class="w-full text-xs"><thead><tr class="text-zinc-600 border-b border-white/5"><th class="text-left py-3 px-6 font-medium">Date</th><th class="text-right py-3 px-6 font-medium">Max</th><th class="text-right py-3 px-6 font-medium">Min</th></tr></thead><tbody>${p.map(d=>`<tr class="border-b border-white/5 hover:bg-white/[0.01]"><td class="py-2.5 px-6 text-zinc-300 font-mono text-[13px]">${d.d}</td><td class="py-2.5 px-6 text-right font-mono text-[13px] ${d.tmax!=null?'text-orange-400':'text-zinc-700'}">${d.tmax!=null?d.tmax.toFixed(1)+'\u00b0':'-'}</td><td class="py-2.5 px-6 text-right font-mono text-[13px] ${d.tmin!=null?'text-blue-400':'text-zinc-700'}">${d.tmin!=null?d.tmin.toFixed(1)+'\u00b0':'-'}</td></tr>`).join('')}</tbody></table><div class="text-center text-xs text-zinc-700 py-4 font-mono">${s+1}\u2013${e} of ${raw.length.toLocaleString()}</div>`;
}
